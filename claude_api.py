...
async def chat(
    ...
    pending_files = []  # Files generated by tools (PDFs etc)
    ...
    try:
        for loop_i in range(max_tool_loops):
            ...
            if response.stop_reason == "tool_use":
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        ...
                        if status_callback:
                            ...
                            elif block.name.startswith("proposal_"):
                                await status_callback(f"📄 Генерирую КП...")

                        result = await _execute_tool_call(block.name, block.input)

                        # Check if tool returned a file (PDF)
                        try:
                            result_data = json.loads(result) if isinstance(result, str) else result
                            if isinstance(result_data, dict) and result_data.get("_has_file"):
                                # Store file for bot to send later
                                pending_files.append({
                                    "data": result_data.get("pdf_base64", ""),
                                    "filename": result_data.get("_filename", "document.pdf"),
                                })
                                # Strip base64 from tool result to save tokens
                                result_data_clean = {k: v for k, v in result_data.items()
                                                      if k not in ("pdf_base64", "_has_file", "_filename")}
                                result_data_clean["file_generated"] = True
                                result = json.dumps(result_data_clean, ensure_ascii=False)
                ...
                return response_text, total_input_tokens, total_output_tokens, pending_files

        # If we hit max loops, return what we have
        return "(Tool loop limit reached)", total_input_tokens, total_output_tokens, pending_files
