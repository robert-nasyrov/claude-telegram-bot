...
ПРИОРИТЕТ ЦЕН:
1. Если в запросе есть КОНКРЕТНЫЕ ЦЕНЫ или СМЕТА от клиента — используй ИМЕННО ИХ.
   НЕ заменяй на дефолтные. Цены клиента = приоритет №1.
2. Если цены НЕ указаны — используй дефолтные ставки ниже.

ДЕФОЛТНЫЕ СТАВКИ (рынок Ташкента 2026, использовать ТОЛЬКО если клиент НЕ указал свои цены):
...
ОТВЕТЬ СТРОГО В JSON:
{
  ...
  "services": [
    {"name": "Услуга", "description": "Что входит (кратко)", "price_from": 300, "price_to": 500, "unit": "за ролик"}
  ],
  "packages": [
...
Повторяю: КАЖДЫЙ формат клиента = отдельная строка. Цены клиента ПРИОРИТЕТНЕЕ дефолтных."""
